...
class Median(val w:Int, k:Int, d:Int) extends Module {
  val io = new Bundle {
    val in = SortData(INPUT, k, d)
    val median = SortData(OUTPUT, k, d)
    val e = Bool(INPUT)
    val v = Bool(OUTPUT)
  }
  val bitonic = Module(new Bitonic(w, k, d))
  val window = Vec.fill(w) { Reg(SortData(k, d)) }
  val ereg = Reg( Bool(false) )
  for (i <- 1 until w)
    window(i) := window(i-1)
  window(0) := io.in
  ereg := io.e
  // Bitonic inputs
  bitonic.io.in := window
  bitonic.io.e := ereg
  // Bitonic outputs
  io.median.key := (bitonic.io.out(w/2).key + bitonic.io.out(w/2-1).key) >> UInt(1)
  io.v := bitonic.io.v
}

class MedianTests(c: Median, n:Int, k:Int, d:Int) extends Tester(c, Array(c.io)) {
  defTests {
    val rnd  = new Random()
...
object Median {
    ...
    chiselMainTest(args, () => Module(new Median(w = 16, k = 32, d = 0))) {
      c => new MedianTests(c, 16, 32, 0) }
  }
}
